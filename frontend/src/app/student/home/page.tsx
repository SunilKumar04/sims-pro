...
import { feesApi, homeworkApi, noticesApi, sessionsApi } from '@/lib/api';
...
export default function StudentHome() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      sessionsApi.getMyStudentSummary(className || undefined).catch(() => ({ data: { data: null } })),
      homeworkApi.getAll({ className }).catch(() => ({ data: { data: [] } })),
      noticesApi.getAll({ limit: 4, target: 'Students' }).catch(() => ({ data: { data: [] } })),
      studentId ? feesApi.getByStudent(studentId).catch(() => ({ data: { data: [] } })) : Promise.resolve({ data: { data: [] } }),
    ]).then(([summaryRes, h, n, feeRes]) => {
      const overall = summaryRes.data?.data?.overall;
      const total = overall?.totalSessions ?? 0;
      const present = overall?.totalPresent ?? 0;
      ...
      setFees(feeRes.data.data?.[0] || null);
    }).finally(() => setLoading(false));
  }, [user?.className, user?.id, user?.studentId]);
  ...
  const attendPct = attendance.percentage;
  const attendColor = attendPct >= 75 ? '#86EFAC' : attendPct >= 60 ? '#FCD34D' : '#FCA5A5';

  const feeStatus = fees?.status || 'PENDING';
        ...
        <div className="relative z-10">
          ...
          <div className="grid grid-cols-2 gap-3 sm:flex sm:flex-wrap sm:gap-8">
            {[
              ...
              { value:attendance.present, label:'Classes Present', color:'#FFFFFF' },
              { value:attendance.absent, label:'Classes Absent', color:'#FCA5A5' },
        ...
        <div className="glass rounded-2xl p-6">
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div className="p-3 rounded-xl text-center" style={{background:'rgba(255,255,255,0.04)'}}>
              <div className="text-sm font-black text-white">₹{(fees?.amount || 0).toLocaleString()}</div>
              <div className="text-xs mt-0.5" style={{color:'rgba(255,255,255,0.35)'}}>Total Amount</div>
            </div>
            <div className="p-3 rounded-xl text-center" style={{background:'rgba(255,255,255,0.04)'}}>
              <div className="text-sm font-black" style={{color:feeColor}}>₹{((fees?.amount || 0) - (fees?.paid || 0)).toLocaleString()}</div>
