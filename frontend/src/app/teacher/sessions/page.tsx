...
export default function TeacherSessions() {
  ...
  const removeSession = async (s: any) => {
    const ok = await confirm({
      ...
      message: `Remove ${s.subject} (${s.className}) from ${new Date(s.date).toLocaleDateString('en-IN')} history? This cannot be undone.`,
      confirm: 'Remove',
      danger: true,
    });
    if (!ok) return;
    ...
    try {
      ...
      toast.success('Removed', 'Session deleted from history');
      await loadHistory();
      void reload();
    } catch (e: any) {
      toast.error('Remove Failed', e?.response?.data?.message ?? e?.message ?? 'Could not remove session');
    } finally {
      setDeletingId('');
    }
  };
