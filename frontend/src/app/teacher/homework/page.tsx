...
export default function TeacherHomework() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        await homeworkApi.update(editingId, form);
        toast.success('Homework Updated', form.title);
      } else {
  ...
  const handleEdit = (h: any) => {
    ...
    setForm({
      title: h.title ?? '',
      subject: h.subject ?? 'Mathematics',
      className: h.className ?? '10A',
  ...
  return (
    <AppShell title="Homework" subtitle="Assign and manage student homework">
      ...
      <div className="glass rounded-2xl p-4 mb-5 flex flex-wrap items-center justify-between gap-4">
        ...
        <button onClick={()=>{setEditingId(null);setForm({subject:'Mathematics',className:'10A'});setModal(true);}}
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => handleEdit(h)}
                              ...
                              style={{background:'rgba(30,144,255,0.12)',color:'#93C5FD',border:'1px solid rgba(30,144,255,0.2)'}}>
                        ✏️ Edit
                      </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="w-full max-w-lg rounded-3xl shadow-2xl" style={{background:'#0F2044',border:'1px solid rgba(255,255,255,0.1)'}}>
            <div className="flex items-center justify-between px-8 pt-7 pb-5">
              <h2 className="text-xl font-extrabold">{editingId ? '✏️ Edit Homework' : '+ Assign Homework'}</h2>
              <button onClick={()=>{setModal(false);setEditingId(null);}} className="w-8 h-8 rounded-xl glass flex items-center justify-center" style={{color:'rgba(255,255,255,0.5)'}}>✕</button>
            </div>
