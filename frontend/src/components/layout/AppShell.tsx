...
const NAV: Record<string, { section: string; items: { id: string; icon: string; label: string; href: string }[] }[]> = {
  ADMIN: [
    ...
    { section:'Academic',      items:[
      ...
      { id:'marks',             icon:'📝',  label:'Marks View',         href:'/admin/marks'         },
      ...
      { id:'reportcard',        icon:'🏅',  label:'Report Cards',       href:'/admin/report-card'   },
    ]},
  ...
  STUDENT: [
    ...
    { section:'Academic',      items:[
      ...
      { id:'sreportcard',       icon:'🏅',  label:'Report Card',        href:'/student/report-card'        },
    ]},
...
const ROLE_LABELS: Record<string,string> = { ADMIN:'School Panel', TEACHER:'Teacher Panel', STUDENT:'Student Panel', PARENT:'Parent Panel' };
const SIDEBAR_W = 248;
  ...
  useEffect(() => {
    ...
    noticesApi.getAll({ limit: 5 }).then(r => {
      ...
      setNotifications(notices.slice(0,5).map((n:any) => ({
        ...
        href: String(u.role).toUpperCase() === 'ADMIN'
          ? '/admin/notices'
