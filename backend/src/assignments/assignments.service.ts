...
export class AssignmentsService {
  ...
  async create(dto: any, rawTeacherId: string) {
    ...
    const data = await this.prisma.assignment.create({
      data: { teacherId, className: dto.className, subject: dto.subject, title: dto.title, description: dto.description ?? '', dueDate: new Date(dto.dueDate), maxMarks: dto.maxMarks ?? 10 },
  ...
  async update(id: string, dto: any) {
    const data = await this.prisma.assignment.update({
      where: { id },
      data: { ...(dto.title && { title: dto.title }), ...(dto.description !== undefined && { description: dto.description }), ...(dto.dueDate && { dueDate: new Date(dto.dueDate) }), ...(dto.maxMarks && { maxMarks: dto.maxMarks }) },
  ...
  async submit(assignmentId: string, rawStudentId: string, dto: any) {
    ...
    if (existing) {
      const data = await this.prisma.assignmentSubmission.update({
        ...
        data: { remarks: dto.remarks ?? '', fileUrl: dto.fileUrl ?? null, submittedAt: new Date(), status: isLate ? 'LATE' : 'SUBMITTED' },
      });
      return { success: true, data };
    }
    const data = await this.prisma.assignmentSubmission.create({
      data: { assignmentId, studentId, remarks: dto.remarks ?? '', fileUrl: dto.fileUrl ?? null, status: isLate ? 'LATE' : 'SUBMITTED' },
    });
    return { success: true, data };
  }
